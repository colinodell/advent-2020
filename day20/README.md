...
When looking for this pattern in the image, *the spaces can be anything*; only the `#` need to match. Also, you might need to rotate or flip your image before it's oriented correctly to find sea monsters. In the above image, *after flipping and rotating it* to the appropriate orientation, there are *two* sea monsters (marked with `<em>O</em>`):
